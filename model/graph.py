...
class SelfAttention(nn.Module):
    ...
    def forward(self, hidden_states,
        ...
        hidden_states = self.dropout(hidden_states)   # [1,61,768]
...
class BartAttention(nn.Module):
    ...
    def forward(
        ...
        src_len = key_states.size(1)   # num_labels
...
class GraphEncoder(nn.Module):
    def __init__(self, config, graph=False, layer=1, data_path=None, threshold=0.01, tau=1, label_dict=None):
        ...
        # self.hir_layers = nn.ModuleList([GraphLayer(config) for i in range(layer)])
