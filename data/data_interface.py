...
class BertDataset_rcv(Dataset):
    def __init__(self, max_token=512, device='cpu', pad_idx=0, data_path=None, label_dict=None, is_test=False,
        ...
        num_lines = sum(1 for line in open(data_path,'r', encoding='utf-8'))
...
class DInterface(pl.LightningDataModule):
    def __init__(self, args, tokenizer, label_depths, data_path, device, label_dict, positive_threshold: int = 5,
        hard_negative_threshold: int = 10, easy_negative_threshold: int = 30):
        super().__init__()
        self.args = args
    ...
    def train_dataloader(self):
        if not self.skip_batch_sampling:
            return DataLoader(self.train_dataset, sampler=self.train_sampler, batch_size=1, collate_fn=self.dataset.collate_fn_1, num_workers=8)
        else:
            return DataLoader(self.train_dataset, batch_size=self.args.batch, shuffle=False, collate_fn=self.dataset.collate_fn, num_workers=8)
